		...
		>
			<div className="fixed h-screen w-screen inset-0 z-50 bg-pale-yellow bg-opacity-75 md:overflow-auto">
				<div className="z-50 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 m-0 w-8/12 md:w-full h-min bg-fgreen">
					<button
						onClick={onClose}
						className="text-left relative md:absolute md:top-16 top-8 left-8 hover:opacity-50 z-50 text-pale-yellow transition duration-300 ease-in-out"
					>
						Back
					</button>
					...
					<div className="flex justify-center m-auto md:block">
						...
						<div className="w-1/3 py-auto mx-8 my-16 md:w-3/4">
							<h1 className="text-4xl md:text-2xl text-pale-yellow">
								{capitaliseFirstLetter(bookTitle)}
							</h1>
							<h4 className="py-4 hover:opacity-50 cursor-pointer text-pale-yellow transition duration-300 ease-in-out">
								{bookAuthor}
							</h4>
							<p className="pb-4 text-pale-yellow text-sm">{bookDescription}</p>
							<hr className="my-4"/>
							<h3 className="pb-2 text-pale-yellow">Check it out on:</h3>
							<h4>
								<a
									className="hover:opacity-50 text-pale-yellow transition duration-300 ease-in-out font-sans-serif"
									href={amazon}
									target="_blank"
								>
									Amazon
							...
							<h4 className="pt-1">
								<a
									className="hover:opacity-50 text-pale-yellow transition duration-300 ease-in-out font-sans-serif"
									href={`https://www.goodreads.com/search?q=${bookTitle}`}
									target="_blank"
								>
									Good Reads
								</a>
							...
							<button
								data-tip={
									shelf.some((title) => title[0] === bookTitle)
							...
							<ReactTooltip
								className="!bg-pale-yellow !rounded-none !ml-6 !text-fgreen"
								arrowColor="!pale-yellow"
								place="right"
							/>
						</div>
					</div>
