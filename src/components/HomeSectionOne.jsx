...
const HomeBestseller = () => {
  ...
  //! Right Column
  const bestsellerRight = data?.results?.lists[0]?.books
    ?.slice(1, 5)
    .map((item, i) => {
      return (
        <div
  ...
  //! Left Column
  const bestsellerLeft = data?.results?.lists[0]?.books
    ?.slice(0, 1)
    .map((item, i) => {
      return (
        <div
  ...
  return (
    <>
      <div className="flex m-auto md:block">
        ...
        <div className="mx-8 md:mx-2 flex justify-start md:justify-center flex-wrap w-6/12 md:w-auto md:my-4">
          {bestsellerRight}
        </div>
      </div>
    </>
