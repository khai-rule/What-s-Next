...
const Bookshelf = ({ removeFromShelf, shelf }) => {
	...
	const displayItems = () => {
		const toDisplay = shelf.map((item, i) => {
			...
			const amazon = item[3];
			return (
				<div className="flex-shrink-0 w-1/5 md:w-1/3 mx-4 pt-8 mb-16">
					<img key={title} src={img} />
					<h1 className="py-2 pt-3 text-2xl md:text-lg">
						{capitaliseFirstLetter(title)}
					</h1>
					<h4 className="hover:opacity-50 cursor-pointer">{author}</h4>
					<p className="py-2 text-sm">{description}</p>
					<hr className="my-4 border-fgreen"/>
					...
					<h4 className="pt-1">
						<a
							className="hover:opacity-50 transition duration-300 ease-in-out font-sans-serif"
			...
			return (
				<div className="text-center">
					<h2
						...
						data-aos-once="true"
					>
						You have not added anything in your bookshelf
					</h2>
					<button
